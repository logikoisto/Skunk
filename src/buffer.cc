...
//move read pointer 
#define COMPLETE_READ(size) do{\
  read_ptr_ = (read_ptr_ + size) % capcity_; \
  length_ -= size;\
  }while(0)

//move write pointer 
#define COMPLETE_WRITE(size) do{\
  write_ptr_ = (write_ptr_ + size) % capcity_; \
  length_ += size;\
  }while(0)


zoo::skunk::Buffer::Buffer(size_t capcity): capcity_(capcity), length_(0), read_ptr_(0), write_ptr_(0){
  //TODO: the buffer in the next version of skunk will be alloced from the arena module to make effective allocation 
...
size_t zoo::skunk::Buffer::Readable() const{
  return length_;
}

size_t zoo::skunk::Buffer::Writeable() const{
  return capcity_ - length_;
}

size_t zoo::skunk::Buffer::Capcity() const{
  return capcity_;
}
...
size_t zoo::skunk::Buffer::Read(off_t offset, zoo::skunk::Buffer& rhs, size_t len){
  COMPLETE_READ(1);
  return 0;
}

size_t zoo::skunk::Buffer::Read(size_t len, iovec* vectors, size_t&  vector_size){
  COMPLETE_READ(1);
  return 0;
}
